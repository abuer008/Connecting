...
//                HapticEffect.impactFeedback(style: .soft, intensity: 3)
        ...
        //                    .scaleEffect(activeIndex == index ? scaleParm : 1)
            ...
            //                .offset(x: 0, y: showDetailView ? -213 : 0)
                    ...
                    if !isSingleCardBeenActived && !isListState && uiState.activeCharacter.id == character.id {
                        VStack {
                            HStack {
...
struct CharacterCardView_Previews: PreviewProvider {
    ...
    static var previews: some View {
        CharacterCardView(isSingleCardBeenActived: .constant(false), showDetail: .constant(false), isListState: .constant(false), character: .constant(mockCharacters[0]))
    }
}
